...
type DefaultsDeep<T, U extends DeepPartial<T>> = {
  [P in keyof T]-?: NonNil<T[P]> extends NonTraversable
    ...
    ? null extends U[P]
      ? undefined extends U[P]
        ? Converted<DefaultsDeep<T[P], NonNil<U[P]>>> | Undef2
        : Converted<DefaultsDeep<T[P], NonNil<U[P]>>>
      : undefined extends U[P]
      ? Converted<NonNil<DefaultsDeep<T[P], NonNil<U[P]>>>> | Undef
...
export const defaultsDeep = <T, U extends DeepPartial<T>>(object: T, defaults: U): Converted<DefaultsDeep<T, U>> => {
